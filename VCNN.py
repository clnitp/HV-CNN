...
model = gensim.models.Word2Vec(data['sequence'], min_count = 1, vector_size = 100, window = 5, workers = 20)
...
# define model
model = Sequential()
model.add(Conv2D(filters=128, kernel_size=(5,1), activation='relu', input_shape=(n_steps, n_features,1),padding='valid'))
...
# fit model
model.fit(X_train, y_train, epochs=25, validation_split=0.2,verbose=2)
...
plt.savefig("Vertical_accuracy.png")
#plt.show()
plt.close()
